...
Connection.prototype.end = function() {
  ...
  this._enqueue('LOGOUT', function() {
    self._queue = [];
    self._curReq = undefined;
    if (self._sock && self._sock.connected) {
      self._sock && self._sock.end();
    }
  });
};
  ...
  if (type === 'bye')
    ...
    if (/^(?:UID )?FETCH/.test(this._curReq.fullcmd)) {
      ...
      if (msg === undefined) {
        ...
        let infoTextKeys = (info.text && Object.keys(info.text)) || [];

        // only allow unsolicited, untagged FETCH responses if they are NOT flag updates
        // otherwise, this will fire a 'message' event and will confuse listeners waiting for 'message' events
        // from a tagged FETCH response. (See lines 196-207)

        if ((infoTextKeys.length <= 3 && infoTextKeys.some(k => k === 'flags') &&
            !infoTextKeys.some(k => ['flags','uid','modseq'].indexOf(k) === -1))) {
          console.log('Ignoring unsolicted untagged flag update. info.text keys: ' + infoTextKeys);
        } else {
